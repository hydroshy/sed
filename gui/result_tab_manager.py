...
class ResultTabManager:
    ...
    def setup_table(self):
        ...
        try:
            ...
            # Set column count
            self.result_table_view.setColumnCount(4)
            
            # Set headers
            headers = ['Frame ID', 'Sensor IN', 'Sensor OUT', 'Status']
    ...
    def on_sensor_in_received(self, sensor_id_in: int) -> int:
        """
        NEW: Nhận tín hiệu sensor IN từ TCP, ghép với pending result
        
        Gọi từ TCP controller khi nhận start_sensor event.
        Tạo frame mới với sensor_id từ TCP.
        Ghép kết quả job đã lưu với frame mới.
        
        Args:
            sensor_id_in: Sensor ID từ TCP start_sensor event
        ...
        try:
            ...
            # Kiểm tra có pending result không
            if not self.pending_result:
                logger.warning(f"[ResultTabManager] No pending result! Create frame with empty status")
                print(f"DEBUG: [ResultTabManager] No pending result, creating frame with PENDING status")
                # Vẫn tạo frame nhưng status = PENDING
                frame_id = self.add_sensor_in_event(sensor_id_in)
                self.set_frame_status(frame_id, 'PENDING')
                return frame_id
            
            # Tạo frame với sensor_id từ TCP
            frame_id = self.add_sensor_in_event(sensor_id_in)
            logger.info(f"[ResultTabManager] Created frame: frame_id={frame_id}")
            print(f"DEBUG: [ResultTabManager] Created frame {frame_id}")
            
            # Ghép status từ pending result
            status = self.pending_result.status
            self.set_frame_status(frame_id, status)
            logger.info(f"[ResultTabManager] Set frame status: {status}")
            print(f"DEBUG: [ResultTabManager] Set frame status: {status}")
            
            # Ghép detection data nếu có
            if self.pending_result.detection_data:
                self.set_frame_detection_data(frame_id, self.pending_result.detection_data)
                logger.info(f"[ResultTabManager] Stored detection data for frame {frame_id}")
                print(f"DEBUG: [ResultTabManager] Stored detection data")
    ...
    def refresh_table(self):
        ...
        try:
            ...
            for row_idx, item_dict in enumerate(queue_data):
                ...
                # Sensor IN
                sensor_in_item = QTableWidgetItem(str(item_dict['sensor_id_in']))
                ...
                # Sensor OUT
                sensor_out_text = str(item_dict['sensor_id_out']) if item_dict['sensor_id_out'] else '-'
                ...
                if status_text == 'OK':
                    ...
                    print(f"DEBUG: ResultTabManager refresh_table - Row {row_idx}: status=NG (red)")
                else:  # PENDING
                    status_item.setBackground(Qt.yellow)
                    print(f"DEBUG: ResultTabManager refresh_table - Row {row_idx}: status=PENDING (yellow)")
                
                self.result_table_view.setItem(row_idx, self.COLUMNS['status'], status_item)
